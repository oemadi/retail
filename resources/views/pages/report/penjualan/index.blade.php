...
<div class="row kotak-atas">
    @include('pages.report.penjualan.kotak_atas')
</div>
                ...
                <div class="row mt-3">
                    <div class="col-md-12">
                        <div class="table-responsive">
                            @include('pages.report.penjualan.table')
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function(){
        $('#filter1').click(function(){
            ...
            let url = `{{ url('/report/penjualan/loadTable?status=`+status+`&tanggal_awal=`+tanggal_awal+`&tanggal_akhir=`+tanggal_akhir+`') }}`;
        ...
        $('.print').click(function(){
            ...
            status = $('#status').val();
            let url =
            `{{ url('/report/penjualan/print?status=`+ status +`&tanggal_awal=`+tanggal_awal+`&tanggal_akhir=`+tanggal_akhir+`') }}`;
            const parseResult = new DOMParser().parseFromString(url, "text/html");
            const parsedUrl = parseResult.documentElement.textContent;
        ...
        $('.excel').click(function(){
            ...
            status = $('#status').val();
            let url =
            `{{ url('/report/penjualan/excel?status=`+ status +`&tanggal_awal=`+tanggal_awal+`&tanggal_akhir=`+tanggal_akhir+`') }}`;
            const parseResult = new DOMParser().parseFromString(url, "text/html");
            const parsedUrl = parseResult.documentElement.textContent;
            window.open(parsedUrl,'_blank');
        });
